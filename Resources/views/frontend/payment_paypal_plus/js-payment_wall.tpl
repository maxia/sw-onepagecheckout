    ...
    <script type="text/javascript">
        ...
        function paymentWall($, approvalUrl) {
            var isCurrentPaymentMethodPaypal = {if $sUserData.additional.payment.id == $PayPalPaymentId}true{else}false{/if},
                    ...
                    $payPalMethodContainer = $payPalCheckBox.parents('.payment--method').first(),
            ...
            // move or create ppplus container
            var $ppplus = $('#ppplus');

            if ($ppplus.length) {
                $payPalMethodContainer.append('<div id="ppplus">' + $ppplus.html() + '</div>');
                $ppplus.remove();
            }
            else {
                $payPalMethodContainer.append('<div id="ppplus"></div>');
            }
            ...
            ppp = PAYPAL.apps.PPP({
                ...
                onContinue: function () {
                    window.location.href = '{url controller=checkout action=confirm}';
                },
        ...
        jQuery(document).ready(function ($) {
            ...
            $.subscribe('plugin/onePageCheckout/shippingPaymentChanged', function(plugin, data) {
                console.log(data);

                if (data.payment == {$PayPalPaymentId})
                    paymentWall($);
            });
